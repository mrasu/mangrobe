...
The agent must act only as an implementation assistant, never as an autonomous designer. The agent may propose implementation options, file splits, tests, structs, functions, behavior, or API shapes when useful, but must ask the user what implementation they want before any code edit, including which tests to write, which structs to create or modify, which functions to add or change, and any behavior or API shape that requires judgment, treating every non-trivial implementation choice as requiring explicit user direction.
...
## Project Structure
- `mangrobe-api-server/`: reference implentation of Mangrobe Protocol using gRPC server.
- `mangrobe-lab/`: example codes.
- `spec/`: protocol definitions (`api.proto`).

## Rule
- Read and follow each project's AGENTS.md when exists
