...
use crate::proto::{StatisticsValue, statistics_value};

pub(crate) fn build_get_file_info_response(
    request_param: &GetFileInfoParam,
    file_info: Vec<FileWithStatistics>,
) -> GetFileInfoResponse {
    GetFileInfoResponse {
        file_info: file_info
            .iter()
            .map(|file| build_file_info_response(request_param, file))
            .collect(),
    }
}
    ...
    let column_statistics = file_info
        ...
        .map(|file| {
            ...
            if request_param.stat_types.includes_min {
                stat.min = file.min.map(|min| StatisticsValue {
                    value: Some(statistics_value::Value::DoubleValue(min)),
                });
            }
            if request_param.stat_types.includes_max {
                stat.max = file.max.map(|max| StatisticsValue {
                    value: Some(statistics_value::Value::DoubleValue(max)),
                });
            }
