...
use crate::grpc::proto::AddedFile;
...
pub async fn prepare() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut added_files: Vec<AddedFile> = vec![];
...
async fn create_vortex_files(dir: &TempDir) -> Result<Vec<String>, Box<dyn std::error::Error>> {
